...
async function publishSeiraPrisma() {
  console.log('=== INSERTING SEIRA ARTICLES WITH UUID GENERATION (DRAFT STATUS) ===\n');
  ...
  if (customerArticleId) {
    ...
    customerArticleId = crypto.randomUUID();
...
`;
  ...
  if (recruitArticleId) {
    ...
    recruitArticleId = crypto.randomUUID();
    await supabase.from('media_articles').insert([{
      id: recruitArticleId,
      slug: recruitSlug,
      title: recruitTitle,
      content: recruitContent,
...
publishSeiraPrisma().catch(console.error);
