...
export default function PriceManagementClient({ initialStores }: PriceManagementClientProps) {
  ...
  // 初期データがない場合はデフォルトデータを表示（開発用）
  const [stores, setStores] = useState<StoreConfig[]>(
    initialStores.length > 0 ? initialStores : [DEFAULT_STORE],
  );
  const [currentStore, setCurrentStore] = useState<StoreConfig>(
    initialStores.length > 0 ? initialStores[0] : DEFAULT_STORE,
  );
