...
function HeroStoreCastSlider({ stores, casts }: { stores: any[]; casts: any[] }) {
  ...
  const slides = useMemo(() => {
    ...
    return stores.map((store) => {
      ...
      return {
        ...
        areaName:
          store.slug === 'fukuoka' ? '博多・天神・中洲対応' : '関内・みなとみらい・桜木町対応',
        ...
        castImage:
          ...
          topCast?.image_url ||
          topCast?.image ||
          (store.slug === 'fukuoka' ? '/ゆうと.png' : '/シュン.png'),
  ...
  return (
    ...
    >
      <div className="group relative aspect-[4/5] w-full max-w-lg overflow-hidden rounded-3xl border-4 border-white bg-slate-900 shadow-2xl shadow-rose-900/15 sm:aspect-[4/3]">
        {/* 背景画像 ＆ スライドトランジション */}
        ...
        <div className="pointer-events-none absolute left-4 right-4 top-4 z-10 flex items-center justify-between">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-slate-900/80 px-3 py-1 text-xs font-bold text-white shadow-lg backdrop-blur-md">
            <span className="h-2 w-2 animate-ping rounded-full bg-rose-500" />
            <span>{currentSlide.badgeText}</span>
          </div>
          <div className="rounded-full bg-rose-500/90 px-3 py-1 font-serif text-[11px] font-bold text-white shadow-md backdrop-blur-sm">
            {currentSlide.areaName}
          </div>
        </div>

        {/* 🚀 キャスト詳細 ＆ 店舗案内オーバーレイ (タップで店舗へ) */}
        <div className="absolute inset-x-0 bottom-0 z-10 space-y-3 p-5 sm:p-6">
          <AnimatePresence mode="wait">
            <motion.div
              key={currentIndex}
            ...
            >
              ...
              <div className="flex items-baseline gap-2">
                <h3 className="font-serif text-2xl font-black text-white drop-shadow-md sm:text-3xl">
              ...
              {/* キャッチコピー */}
              <p className="line-clamp-2 text-xs font-medium text-slate-200 drop-shadow sm:text-sm">
          ...
          <div className="flex items-center justify-between border-t border-white/15 pt-2">
            <Link
              ...
              className="group/btn inline-flex items-center gap-1.5 text-xs font-bold text-rose-300 transition hover:text-white"
            >
...
export default function HubPageClient({
  ...
  return (
    <div className="min-h-screen w-full max-w-[100vw] overflow-x-hidden bg-slate-50 text-slate-900 selection:bg-rose-500/30">
      ...
      <section
        aria-label="ストロベリーボーイズの信頼と実績"
        className="border-b border-rose-100/60 bg-white py-4"
      >
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-2 px-4 sm:gap-3">
          {showReviewCount && (
            <span className="inline-flex items-center rounded-full border border-rose-100 bg-rose-50 px-3.5 py-1.5 text-[11px] font-bold text-rose-600 sm:text-xs">
              お客様の口コミ {reviewCount!.toLocaleString()}件
            </span>
          )}
          <span className="inline-flex items-center rounded-full border border-rose-100 bg-rose-50 px-3.5 py-1.5 text-[11px] font-bold text-rose-600 sm:text-xs">
            Since 2018・グループ運営8年
      ...
      <section
        id="about"
        className="relative overflow-hidden border-t border-rose-100/60 bg-white py-16 sm:py-24"
      >
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 rounded-full border border-rose-100 bg-rose-50 px-4 py-1.5 text-xs font-bold text-rose-600">
              <Crown className="h-3.5 w-3.5 text-rose-500" />
              ABOUT STRAWBERRY BOYS わたしたちについて
            </div>

            <h3 className="font-serif text-3xl font-black leading-tight text-slate-900 sm:text-4xl lg:text-5xl">
              安心できるから、
              <br />
            ...
            <p className="mx-auto max-w-2xl text-sm font-medium leading-relaxed text-slate-600 sm:text-base">
              仕事も、日常も、いつも頑張っているあなたへ。
              <br className="hidden sm:inline" />
              癒やされたい夜も、誰かに甘えたい夜も、ここでは、あなたの気持ちとペースを大切にします。
              <br />
            ...
            {/* 4つの安心カード */}
            <div className="grid w-full max-w-3xl grid-cols-1 gap-4 pt-6 text-left sm:grid-cols-2">
              <div className="flex items-start gap-3.5 rounded-2xl border border-rose-100/80 bg-[#FAF6F3] p-4 shadow-sm sm:p-5">
                <div className="shadow-xs flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-rose-500">
            ...
            <div className="flex w-full flex-col items-center justify-center gap-4 pt-6 sm:flex-row">
              <Link
                ...
                className="flex w-full items-center justify-center gap-2 rounded-full bg-rose-500 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-rose-500/20 transition hover:bg-rose-600 sm:w-auto"
              >
              ...
              <a
                href="#faq"
                className="flex items-center gap-1.5 py-2 text-xs font-bold text-slate-700 underline underline-offset-4 transition hover:text-rose-500"
              >
      ...
      {(() => {
        ...
        const filteredFeatured = (featuredCasts || []).filter((fc: any) => {
          ...
          return (
            slug === 'fukuoka' ||
            slug === 'yokohama' ||
            (fc.store_name && !fc.is_external && !['osaka', 'nagoya'].includes(slug))
          );
        });
        ...
        const displayCasts =
          filteredFeatured.length > 0
            ...
            : validCasts.map((c) => ({
                ...
                name: c.name,
                store_name:
                  c.stores?.[0]?.name || (c.storeSlug === 'yokohama' ? '横浜店' : '福岡店'),
        ...
        const marqueeCasts =
          sortedCasts.length >= 8 ? sortedCasts : [...sortedCasts, ...sortedCasts];

        return (
          <section className="overflow-hidden bg-slate-50 py-24 [contain-intrinsic-size:500px] [content-visibility:auto]">
            <div className="mx-auto max-w-7xl px-6 text-center">
              <motion.div
                ...
                className="mb-12"
              >
                <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50 px-3.5 py-1 text-xs font-bold text-rose-600">
                  ✨ 全国の人気セラピストをピックアップ
                </div>
            ...
            <div className="relative mt-4 flex overflow-hidden before:absolute before:left-0 before:top-0 before:z-10 before:h-full before:w-24 before:bg-gradient-to-r before:from-slate-50 before:to-transparent after:absolute after:right-0 after:top-0 after:z-10 after:h-full after:w-24 after:bg-gradient-to-l after:from-slate-50 after:to-transparent">
              <div className="animate-scroll-text flex cursor-pointer space-x-8 whitespace-nowrap py-6 hover:[animation-play-state:paused]">
                {marqueeCasts.map((cast: any, i: number) => {
                  const isExt = Boolean(
                    cast.is_external || ['tokyo', 'osaka', 'nagoya'].includes(cast.store_slug),
                  );
                  const storeName =
                    cast.store_name ||
                    (cast.store_slug === 'yokohama'
                      ? '横浜店'
                      : cast.store_slug === 'fukuoka'
                        ? '福岡店'
                        : 'グループ店');
                  ...
                  const CardInner = (
                    <div className="group">
                      ...
                      <div className="px-2 text-center">
                        <div className="shadow-xs mb-1 inline-flex items-center gap-1 rounded-full bg-slate-900/80 px-2.5 py-0.5 text-[9px] font-bold text-white">
                        ...
                        <p className="mx-auto mt-0.5 max-w-[160px] truncate text-[10px] font-bold text-rose-400 opacity-90">
                  ...
                  return (
                    <div
                      key={`${cast.id || cast.name}-${i}`}
                      className="inline-block w-44 shrink-0 text-center"
                    >
                      {isExt ? (
                        <a
                          href={cast.link_url}
      ...
      <section className="bg-white px-6 py-24 [contain-intrinsic-size:500px] [content-visibility:auto]">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="no-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto pb-6 md:gap-8">
            {diaries.length > 0
              ? diaries.slice(0, 4).map((diary, i) => {
                  ...
                  const storeSlug =
                    (diary as any).store_slug ||
                    (diary as any).storeSlug ||
                    (castData as any)?.store_slug ||
                    'fukuoka';
          ...
          <div className="border-t border-slate-100 pt-4">
            <p className="mb-3 text-center text-xs font-bold text-slate-500 sm:text-left">
              各店舗の最新写メ日記はこちらから
            </p>
            <div className="flex flex-wrap gap-2 text-xs font-bold">
              <Link
                href="/store/fukuoka/diary"
                className="shadow-xs rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-rose-600 transition hover:bg-rose-100"
              >
                福岡店の写メ日記一覧 →
              </Link>
              <Link
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="shadow-xs rounded-full border border-slate-200 bg-white px-4 py-2 text-slate-700 transition hover:bg-slate-100"
              >
                東京店 外部サイト ↗
              </a>
              <a
      ...
      {/* ─── 2. コンテンツ切り替えタブ ─── */}
      <section className="bg-white px-6 py-20 [contain-intrinsic-size:500px] [content-visibility:auto]">
        <div className="mx-auto max-w-7xl">
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
                  {(newsPages && newsPages.length > 0
                    ...
                    const storeSlug =
                      news.store_slug ||
                      news.storeSlug ||
                      (news.store_id === 'yokohama' ? 'yokohama' : 'fukuoka');
                    const dateRaw =
                      news.storeSettings?.[storeSlug]?.publishedAt ||
                      (Object.values(news.storeSettings || {}) as any[]).find(
                        (s: any) => s?.publishedAt,
                      )?.publishedAt ||
                      news.publishedAt ||
                      news.published_at ||
                    ...
                    const newsHref = news.slug
                      ? `/store/${storeSlug}/news/${news.slug}`
                      : `/store/${storeSlug}/news`;

                    return (
                      <Link href={newsHref} key={news.id || idx} className="group block">
                        <motion.div
                          whileHover={{ y: -6 }}
      ...
      {/* ─── 4. Knowledge Hub ─── */}
      <section className="bg-slate-50 px-6 py-24 [contain-intrinsic-size:500px] [content-visibility:auto]">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {GUIDE_ARTICLES.map((article) => (
              <Link key={article.title} href={article.href} className="group block">
                <motion.div
                  whileHover={{ y: -6 }}
                  className="flex h-full flex-col justify-between rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm transition-all group-hover:border-rose-200 group-hover:shadow-md"
                >
                  <div>
                    <div className="mb-6 text-4xl">{article.icon}</div>
                    <h3 className="mb-2 text-lg font-black text-slate-800 transition-colors group-hover:text-rose-500">
                      {article.title}
                    </h3>
                    <p className="text-sm leading-relaxed text-slate-400">{article.desc}</p>
                  </div>
                  <div className="mt-4 flex items-center justify-between border-t border-slate-50 pt-4 text-xs font-bold text-rose-500">
                    <span>詳しく読む</span>
                    <span>→</span>
                  </div>
                </motion.div>
      ...
      <section className="bg-gradient-to-b from-white to-pink-50/50 px-6 py-24 [contain-intrinsic-size:500px] [content-visibility:auto]">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12 text-center">
            <span className="mb-3 inline-block rounded-full bg-rose-100 px-4 py-1.5 text-xs font-black tracking-wider text-rose-600">
              CLEAR PRICING & TRUST
            </span>
            <h2 className="text-3xl font-black text-slate-900 md:text-5xl">
              明朗会計・安心の料金プラン
            </h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm font-bold leading-relaxed text-slate-500">
            ...
            <div className="mt-8 flex justify-center gap-3">
              <button
                ...
                className={`rounded-full px-6 py-2.5 text-xs font-black transition-all sm:text-sm ${
                  priceTab === 'fukuoka'
          ...
          <div className={`${priceTab === 'fukuoka' ? 'block' : 'hidden'}`}>
            <div className="mb-6 text-center">
              <span className="text-xs font-bold text-slate-500">
                【ストロベリーボーイズ福岡店 基本コース価格】
              </span>
            </div>
            <div className="mb-10 grid grid-cols-1 gap-6 md:grid-cols-3">
              <div className="flex flex-col justify-between rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-lg transition-all hover:shadow-xl">
                <div>
                  <div className="mb-4 inline-block rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">
                    お試しショート
                  </div>
                  <h3 className="mb-2 text-2xl font-black text-slate-900">60分コース</h3>
                  <p className="mb-6 text-xs text-slate-400">
                    初めての方や短時間で癒やされたい方に
                  </p>
                  <div className="mb-6 flex items-baseline gap-1">
                    <span className="text-4xl font-black text-rose-500">
                      ¥{(storePrices?.fukuoka?.[0]?.price || 12000).toLocaleString()}
                    </span>
                ...
                <ul className="space-y-2.5 border-t border-slate-100 pt-6 text-xs font-bold text-slate-600">
                  ...
                  <li className="flex items-center gap-2">
                    <span className="text-rose-500">✓</span> 福岡市内・博多・天神エリア対応
                  </li>
                </ul>
              </div>

              <div className="flex transform flex-col justify-between rounded-[2.5rem] border-2 border-rose-400 bg-white p-8 shadow-2xl transition-all md:-translate-y-2">
                <div>
                  <div className="mb-4 inline-block rounded-full bg-rose-100 px-3 py-1 text-xs font-black text-rose-600">
                    人気No.1 標準プラン
                  </div>
                  <h3 className="mb-2 text-2xl font-black text-slate-900">90分コース</h3>
                  <p className="mb-6 text-xs text-slate-400">じっくり全身の施術と会話を満喫</p>
                  <div className="mb-6 flex items-baseline gap-1">
                    <span className="text-4xl font-black text-rose-500">
                      ¥{(storePrices?.fukuoka?.[1]?.price || 16000).toLocaleString()}
                    </span>
                    <span className="text-sm font-bold text-slate-400">〜（税込）</span>
                  </div>
                </div>
                <ul className="space-y-2.5 border-t border-slate-100 pt-6 text-xs font-bold text-slate-600">
                  <li className="flex items-center gap-2">
                    <span className="text-rose-500">✓</span> 人気No.1の満足コース
                  </li>
                  <li className="flex items-center gap-2">
              ...
              <div className="flex flex-col justify-between rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-lg transition-all hover:shadow-xl">
                <div>
                  ...
                  <h3 className="mb-2 text-2xl font-black text-slate-900">120分コース</h3>
                  <p className="mb-6 text-xs text-slate-400">時間を忘れて最高峰の癒やしを体験</p>
                  <div className="mb-6 flex items-baseline gap-1">
                    <span className="text-4xl font-black text-rose-500">
                      ¥{(storePrices?.fukuoka?.[2]?.price || 20000).toLocaleString()}
                    </span>
                    <span className="text-sm font-bold text-slate-400">〜（税込）</span>
                  </div>
                </div>
                <ul className="space-y-2.5 border-t border-slate-100 pt-6 text-xs font-bold text-slate-600">
                  <li className="flex items-center gap-2">
                    <span className="text-rose-500">✓</span> 特別な夜のためのロングコース
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="text-rose-500">✓</span> ご要望に応じたオーダーメイド施術
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="text-center">
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full bg-rose-500 px-8 py-3.5 text-xs font-bold text-white shadow-lg transition hover:bg-rose-600 sm:text-sm"
              >
            ...
            <div className="mb-10 grid grid-cols-1 gap-6 md:grid-cols-3">
              <div className="flex flex-col justify-between rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-lg transition-all hover:shadow-xl">
                <div>
                  ...
                  <div className="mb-6 flex items-baseline gap-1">
                    <span className="text-4xl font-black text-rose-500">
                      ¥{(storePrices?.yokohama?.[0]?.price || 12000).toLocaleString()}
                    </span>
                    <span className="text-sm font-bold text-slate-400">〜（税込）</span>
                  </div>
                </div>
                <ul className="space-y-2.5 border-t border-slate-100 pt-6 text-xs font-bold text-slate-600">
                  ...
                  <li className="flex items-center gap-2">
                    <span className="text-rose-500">✓</span> 横浜・関内・みなとみらいエリア対応
                  </li>
                </ul>
              </div>
              ...
              <div className="flex transform flex-col justify-between rounded-[2.5rem] border-2 border-rose-400 bg-white p-8 shadow-2xl transition-all md:-translate-y-2">
                <div>
                  ...
                  <div className="mb-6 flex items-baseline gap-1">
                    <span className="text-4xl font-black text-rose-500">
                      ¥{(storePrices?.yokohama?.[1]?.price || 16000).toLocaleString()}
                    </span>
                    <span className="text-sm font-bold text-slate-400">〜（税込）</span>
                  </div>
                </div>
              ...
              <div className="flex flex-col justify-between rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-lg transition-all hover:shadow-xl">
                <div>
                  ...
                  <div className="mb-6 flex items-baseline gap-1">
                    <span className="text-4xl font-black text-rose-500">
                      ¥{(storePrices?.yokohama?.[2]?.price || 20000).toLocaleString()}
                    </span>
                    <span className="text-sm font-bold text-slate-400">〜（税込）</span>
                  </div>
                </div>
          ...
          {/* 外部系列店舗の案内リンク行 */}
          <div className="mt-12 border-t border-rose-100/60 pt-6 text-center text-xs font-bold text-slate-500">
            <span>
              ※東京・大阪・名古屋など系列他店舗の料金につきましては、各店舗の公式サイトをご確認ください:
            </span>
          ...
          {/* 信頼バッジ ＆ 料金ページへの導線 */}
          <div className="mt-12 flex flex-col items-center justify-between gap-6 rounded-3xl border border-rose-100 bg-white p-8 shadow-sm md:flex-row">
            <div className="flex flex-wrap items-center gap-6 text-center md:text-left">
              <div className="flex items-center gap-3">
                <Lock className="h-6 w-6 shrink-0 text-rose-500" />
                <div>
              ...
              <div className="flex items-center gap-3">
                <Calculator className="h-6 w-6 shrink-0 text-rose-500" />
                <div>
            ...
            <div className="flex shrink-0 flex-col items-center gap-3 sm:flex-row">
              <Link
                href="/store/fukuoka/price"
                className="flex items-center gap-1.5 rounded-full bg-rose-500 px-6 py-3.5 text-xs font-black text-white transition-all hover:bg-rose-600 hover:shadow-lg hover:shadow-rose-500/30"
              >
              ...
              <Link
                href="/store/yokohama/price"
                className="flex items-center gap-1.5 rounded-full bg-slate-800 px-6 py-3.5 text-xs font-black text-white transition-all hover:bg-slate-900 hover:shadow-lg"
              >
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-12 md:grid-cols-5">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm font-bold text-slate-500">
              <li>
                <Link href="/store/fukuoka" className="transition-colors hover:text-rose-500">
                  福岡店（博多・天神）
                </Link>
              </li>
              <li>
                <Link href="/store/yokohama" className="transition-colors hover:text-rose-500">
                  横浜店（関内・桜木町）
                </Link>
              </li>
              <li>
                <Link href="/guide" className="transition-colors hover:text-rose-500">
                  初めての方へ
                </Link>
              </li>
              <li>
                <Link href="/plan" className="transition-colors hover:text-rose-500">
                  料金・プラン解説
                </Link>
              </li>
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm font-bold text-slate-500">
              <li>
                <Link href="/guide/cast-select" className="transition-colors hover:text-rose-500">
                  キャストの選び方
                </Link>
              </li>
              <li>
                <Link href="/guide/nomination" className="transition-colors hover:text-rose-500">
                  指名のご案内
                </Link>
              </li>
              <li>
                <Link
                  href="/store/fukuoka/contact"
                  className="transition-colors hover:text-rose-500"
                >
                  お問い合わせ
                </Link>
