...
export default function ReservationPage() {
  ...
  useEffect(() => {
    const fetchCasts = async () => {
      try {
        const castsData = await getCastsByStore(slug as string);
        setCasts(castsData.filter((cast) => cast.isActive));
      } catch (error) {
          ...
          >
            ...
            <div className="text-center">
              ...
              <div className="text-lg font-bold text-gray-800">
                {store.contact?.phone || '03-XXXX-XXXX'}
              </div>
            </div>
          </a>

          <a
            href={`https://line.me/R/ti/p/${(store.contact?.line || '@example').replace('@', '')}`}
            target="_blank"
        ...
        >
          <div className="space-y-6">
            ...
            <div>
              ...
              {loading ? (
                <div className="rounded-xl border-2 border-gray-200 px-4 py-3 text-gray-400">
                  読み込み中...
                </div>
              ) : (
                <select
