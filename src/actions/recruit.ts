...
export async function saveRecruitPageConfig(slug: string, config: any) {
  ...
  try {
    ...
    await prisma.recruitPageHistory.create({
      data: {
        store_id: store.id,
    ...
    try {
      const backupDir = path.join(process.cwd(), 'src/lib/store/backups/recruit', slug);
      ...
      console.error('Recruit backup file creation failed:', err);
    }
...
// 履歴一覧の取得
export async function getRecruitHistory(slug: string) {
  try {
    const store = await prisma.store.findUnique({
      where: { slug }
    });
    ...
    console.error('Failed to fetch recruit history:', error);
    return { success: false, error: '履歴の取得に失敗しました' };
  }
}

// 履歴の削除
export async function deleteRecruitHistory(historyId: string) {
  try {
    await prisma.recruitPageHistory.delete({
      where: { id: historyId }
    });
    return { success: true };
  } catch (error) {
