...
interface CastDiaryContentProps {
  slug: string;
  castName: string;
}

const CastDiaryContent: React.FC<CastDiaryContentProps> = ({ slug, castName }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const posts = await getDiaryPostsByCastId(castData.id, slug);
  ...
  return (
    <div className="container mx-auto px-3 py-4 sm:px-4 sm:py-6 md:py-8">
      ...
      <div className="mb-6 sm:mb-8">
        <div className="rounded-xl bg-gradient-to-r from-pink-100 to-pink-50 p-4 sm:p-6 md:p-8">
          <div className="mb-4 text-center">
            <h1 className="mb-2 text-xl font-bold text-gray-800 sm:text-2xl md:text-3xl">
              {decodedCastName}の写メ日記
            </h1>
            <p className="font-serif text-sm text-gray-600 sm:text-base">
      ...
      <div className="mb-4 flex flex-col justify-between gap-3 sm:mb-6 sm:flex-row sm:items-center">
        ...
        <select
          ...
          className="rounded-lg border border-pink-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500"
        >
      ...
      {sortedPosts.length > 0 ? (
        ...
        <div className="py-8 text-center sm:py-12">
          <div className="rounded-xl border border-pink-100 bg-white p-6 text-center shadow-sm sm:p-8">
            <Calendar className="mx-auto mb-3 text-gray-400" size={32} />
            <h3 className="mb-2 text-lg font-bold">まだ日記がありません</h3>
...
export default CastDiaryContent;
