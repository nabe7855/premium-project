...
const Chatbot: React.FC<ChatbotProps> = ({ isOpen, onClose, storeName, storeSlug }) => {
        ...
        if (userInput === '送信する') {
          ...
          const submitData = async () => {
            try {
              ...
              if (formData.datingApp)
                messageParts.push(`【出会い系アプリ利用歴】\n${formData.datingApp}`);
              if (formData.tattoo) messageParts.push(`【刺青】\n${formData.tattoo}`);
              if (formData.appearance)
                messageParts.push(`【容姿の懸念点】\n${formData.appearance}`);
              if (formData.freeText)
                messageParts.push(`【自己PR・意気込み】\n${formData.freeText}`);
              ...
              setIsTyping(false);
              addModelMessage(
                '通信エラーが発生しました。もう一度「送信する」ボタンを押してください。',
              );
            }
          };
          submitData();
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center justify-between border-b border-slate-800 bg-slate-900/50 p-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-600 font-bold text-white">
            L
          </div>
          <div>
      ...
      <div
        ref={scrollRef}
        className="no-scrollbar flex-grow space-y-6 overflow-y-auto scroll-smooth p-4"
      >
        {messages.map((m, i) => (
          <div
            key={i}
          ...
          >
            <div
              className={`max-w-[80%] rounded-2xl p-4 ${m.role === 'user' ? 'rounded-tr-none bg-amber-600 text-white shadow-lg shadow-amber-900/20' : 'rounded-tl-none border border-slate-700 bg-slate-800 text-slate-100'}`}
            >
                ...
                {Object.entries(STEP_LABELS).map(([key, label]) => (
                  ...
                  >
                    <div className="min-w-0 flex-grow">
                      ...
                      <div className="truncate text-sm">{formData[key] || '---'}</div>
                    </div>
                    <button
                      onClick={() => handleEdit(key as Step)}
                      className="mt-1 rounded bg-slate-700 px-2 py-1 text-[10px] text-slate-300"
                    >
                      修正
                    </button>
                  </div>
      ...
      >
        ...
        {currentStep === 'age' && !isTyping && (
          <div className="mb-4 animate-in slide-in-from-bottom-2">
            ...
            <button
              ...
              className="mt-4 w-full rounded-full bg-amber-600 py-3 font-bold text-white"
            >
              年齢を決定する
            </button>
          </div>
          ...
          <div className="mb-4 flex flex-wrap gap-2 animate-in slide-in-from-bottom-1">
            {currentOptions.map((opt) => (
              <button
                ...
                className="rounded-full border border-slate-700 bg-slate-800 px-4 py-2 text-sm text-white hover:bg-amber-600 active:scale-95"
              >
                {opt}
              </button>
            ))}
        ...
        {currentStep === 'review' && !isTyping && (
          <div className="mb-4">
            <button
              ...
              className="w-full rounded-2xl bg-amber-600 py-4 text-base font-bold text-white shadow-xl shadow-amber-900/20 active:scale-95"
            >
              この内容で応募する
            </button>
          </div>
        ...
        {['done', 'review', 'age', 'height'].indexOf(currentStep) === -1 ? (
          <div className="flex items-end gap-2 rounded-2xl bg-slate-800 p-2 pl-4 transition-all focus-within:ring-2 focus-within:ring-amber-500/50">
            <textarea
              ...
              placeholder={
                currentStep === 'photos' ? '写真を選んで送信してください' : '回答を入力...'
              }
            ...
            <button
              ...
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-600 text-white active:scale-90 disabled:opacity-50"
            >
              {currentStep === 'photos' ? '⬆' : '✈'}
            </button>
            <input
          ...
          currentStep === 'done' && (
            <div className="py-2 text-center">
              <button
                onClick={onClose}
                className="rounded-full bg-slate-800 px-8 py-3 font-bold text-white active:scale-95"
              >
                閉じる
              </button>
            </div>
        ...
        <p className="mt-3 text-center text-[10px] text-slate-500">
          安心のプライバシー管理。お送りいただいた情報は厳格に保管されます。
        </p>
      </div>
