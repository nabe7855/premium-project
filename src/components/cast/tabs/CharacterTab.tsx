...
{/* LINE風Q&A（質問左・回答右） */}
<div className="bg-pink-50 border border-pink-200 rounded-md p-4 shadow-sm space-y-4 mt-8">
  <h3 className="text-red-600 font-bold text-lg mb-4">◆ セラピストQ&A（ロングver.）</h3>
  {[
    {
    ...
    <div key={index} className="flex flex-col gap-2">
      {/* 質問：左吹き出し */}
      <div className="self-start bg-white border border-pink-300 px-4 py-2 rounded-xl rounded-bl-sm max-w-[80%] text-sm text-gray-800 whitespace-pre-line shadow">
        <strong>Q.</strong> {qa.question}
      </div>
      {/* 回答：右吹き出し */}
      <div className="self-end bg-pink-100 px-4 py-2 rounded-xl rounded-br-sm max-w-[80%] text-sm text-gray-800 whitespace-pre-line shadow">
        {qa.answer}
      </div>
    </div>
  ))}
