...
async function main() {
  ...
  const targetFile = args[0] || 'result_1'; // like 'result_2'
  const jsonPath = path.join(process.cwd(), `import_results/${targetFile}.json`);
  ...
  const data = Array.isArray(parsedData) ? parsedData : Object.values(parsedData);
    ...
    try {
      await prisma.lh_hotels.update({
        ...
        data: {
          ...
          ai_pros_cons: item.ai_pros_cons ? JSON.parse(JSON.stringify(item.ai_pros_cons)) : null,
        },
      });
