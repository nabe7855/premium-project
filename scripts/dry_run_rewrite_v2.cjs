...
// Using GEMINI_API_KEY_2 from .env (Alternate key)
...
【出力フォーマット (JSON形式のみ)】
{
  ...
  "ai_reviews": [
    { "title": "オリジナルのタイトル", "body": "生口コミの内容を元に、全く別の文章でリライトした口コミ(200-300字程度)", "score": "4.5" }
  ]
}
`;
...
async function main() {
  ...
  const pending = hotelIds
    .filter(
      (id) => data[id].processing_status === 'pending' || data[id].processing_status === 'failed',
    )
    .slice(0, 5);

  console.log(`Starting Dry Run for ${pending.length} hotels with Key 2...`);
