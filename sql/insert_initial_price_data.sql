-- 料金管理システム初期データ投入
-- 注意: このSQLは create_price_management_tables.sql を実行した後に実行してください
-- 注意: 店舗データ（stores テーブル）が既に存在していることを前提としています
...
DECLARE
  ...
  v_course_standard_id UUID;
  v_course_stay_id UUID;
  v_course_date_id UUID;
  v_course_new_id UUID;
  v_course_couple_id UUID;
  v_course_3p_id UUID;
  v_course_travel_id UUID;
BEGIN
  -- 福岡店のIDを取得
  SELECT id INTO v_store_id FROM stores WHERE slug = 'fukuoka' LIMIT 1;
  
  IF v_store_id IS NULL THEN
    RAISE NOTICE 'Store fukuoka not found. Skipping...';
    RETURN;
  END IF;

  -- price_config作成
  INSERT INTO price_configs (store_id, hero_image_url)
  VALUES (v_store_id, '/料金ページトップ画像.jpg')
  ...
  INSERT INTO courses (
    ...
    0
  ) RETURNING id INTO v_course_standard_id;

  -- スタンダードコースのプラン
  INSERT INTO course_plans (course_id, minutes, price, sub_label, discount_info, display_order) VALUES
    (v_course_standard_id, 60, 12000, NULL, NULL, 0),
    (v_course_standard_id, 90, 16000, NULL, NULL, 1),
    (v_course_standard_id, 120, 20000, NULL, '初回2,000円OFF', 2),
    (v_course_standard_id, 150, 24000, NULL, '初回2,000円OFF', 3),
    (v_course_standard_id, 180, 29000, NULL, '初回2,000円OFF', 4),
    (v_course_standard_id, 240, 39000, NULL, '初回4,000円OFF', 5),
    (v_course_standard_id, 300, 47000, NULL, '初回4,000円OFF', 6);
  ...
  INSERT INTO courses (
    ...
    1
  ) RETURNING id INTO v_course_stay_id;

  INSERT INTO course_plans (course_id, minutes, price, display_order) VALUES
    (v_course_stay_id, 600, 55000, 0),
    (v_course_stay_id, 720, 65000, 1),
    (v_course_stay_id, 840, 75000, 2),
    (v_course_stay_id, 960, 85000, 3),
    (v_course_stay_id, 1080, 95000, 4);
  ...
  INSERT INTO courses (
    ...
    2
  ) RETURNING id INTO v_course_date_id;

  INSERT INTO course_plans (course_id, minutes, price, display_order) VALUES
    (v_course_date_id, 180, 32000, 0),
    (v_course_date_id, 240, 42000, 1),
    (v_course_date_id, 300, 50000, 2);
  ...
  INSERT INTO courses (
    ...
    3
  ) RETURNING id INTO v_course_new_id;

  INSERT INTO course_plans (course_id, minutes, price, display_order) VALUES
    (v_course_new_id, 90, 10000, 0);
  ...
  INSERT INTO courses (
    ...
    4
  ) RETURNING id INTO v_course_couple_id;

  INSERT INTO course_plans (course_id, minutes, price, display_order) VALUES
    (v_course_couple_id, 60, 19000, 0),
    (v_course_couple_id, 90, 28000, 1),
    (v_course_couple_id, 120, 37000, 2);
  ...
  INSERT INTO courses (
    ...
    5
  ) RETURNING id INTO v_course_3p_id;

  INSERT INTO course_plans (course_id, minutes, price, display_order) VALUES
    (v_course_3p_id, 90, 30000, 0),
    (v_course_3p_id, 120, 40000, 1);
  ...
  INSERT INTO courses (
    ...
    6
  ) RETURNING id INTO v_course_travel_id;

  INSERT INTO course_plans (course_id, minutes, price, display_order) VALUES
    (v_course_travel_id, 1440, 100000, 0),
    (v_course_travel_id, 1800, 125000, 1),
    (v_course_travel_id, 2160, 150000, 2),
    (v_course_travel_id, 2880, 200000, 3),
    (v_course_travel_id, 3600, 250000, 4);
  ...
  RAISE NOTICE '福岡店の料金データを投入しました';
END $$;

-- 横浜店にも同様のデータを投入（必要に応じて）
-- 同じ構造で横浜店用のデータを作成できます
