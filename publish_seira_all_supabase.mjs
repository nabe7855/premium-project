...
async function publishSeiraAllSupabase() {
  console.log('=== INSERTING SEIRA ARTICLES VIA SUPABASE (DRAFT STATUS) ===\n');

  // 1. Find Seira cast
  const { data: casts } = await supabase.from('casts').select('id, name, slug').or('slug.eq.-130642,name.ilike.%青空%,name.ilike.%せいら%');
  const cast = casts?.[0];
  const castId = cast?.id;
  const castSlug = cast?.slug || '-130642';
  console.log(`Found Cast: Name="${cast?.name}", ID="${castId}", Slug="${castSlug}"`);

  // 2. Customer Interview Article: seira-interview-vol4 (status: 'draft')
  ...
  const { data: existingCustomer } = await supabase.from('media_articles').select('id').eq('slug', customerSlug).maybeSingle();

  let customerArticleId = existingCustomer?.id;

  if (customerArticleId) {
    ...
    }).eq('id', customerArticleId);
  } else {
    const { data: newCust, error } = await supabase.from('media_articles').insert([{
      slug: customerSlug,
      title: customerTitle,
      category: 'interview',
      status: 'draft',
    ...
    if (error) console.error('Customer article insert error:', error);
    customerArticleId = newCust?.[0]?.id;
  }

  console.log(`✅ Upserted Customer Article: ID="${customerArticleId}", Status="draft"`);

  if (!customerArticleId) {
    console.error('Failed to resolve customerArticleId');
    return;
  }
  ...
  const { data: existingMeta } = await supabase.from('interview_meta').select('id').eq('article_id', customerArticleId).maybeSingle();
  ...
  if (existingMeta) {
    ...
    await supabase.from('interview_meta').insert([{
      article_id: customerArticleId,
      article_type: 'single',
      series_slug: 'cast-interview',
      vol_number: 4,
      area: '福岡',
  ...
  if (castId) {
    const { data: existingLink } = await supabase.from('interview_cast_links').select('id').eq('article_id', customerArticleId).eq('cast_id', castId).maybeSingle();
    if (!existingLink) {
      await supabase.from('interview_cast_links').insert([{
        article_id: customerArticleId,
        cast_id: castId,
  ...
  // 3. Recruit Column Article (Ikeo): seira-35-recruit-story (status: 'draft')
...
`;
  ...
  const { data: existingRecruit } = await supabase.from('media_articles').select('id').eq('slug', recruitSlug).maybeSingle();

  let recruitArticleId = existingRecruit?.id;
  ...
  if (recruitArticleId) {
    await supabase.from('media_articles').update({
      ...
      updated_at: new Date().toISOString()
    }).eq('id', recruitArticleId);
  } else {
    const { data: newRec } = await supabase.from('media_articles').insert([{
      slug: recruitSlug,
      title: recruitTitle,
      content: recruitContent,
      excerpt: recruitDescription,
    ...
    recruitArticleId = newRec?.[0]?.id;
  }

  console.log(`✅ Upserted Recruit Column Article (Ikeo): ID="${recruitArticleId}", Slug="${recruitSlug}", Status="draft"`);

  // 4. Link from fukuoka-recruit-guide to seira-35-recruit-story
  const { data: fukuokaGuide } = await supabase.from('media_articles').select('id, content').eq('slug', 'fukuoka-recruit-guide').maybeSingle();
  ...
  if (fukuokaGuide && fukuokaGuide.content && !fukuokaGuide.content.includes(recruitSlug)) {
    ...
    await supabase.from('media_articles').update({
      content: fukuokaGuide.content + additionLink
    }).eq('id', fukuokaGuide.id);
    console.log('✅ Added link from fukuoka-recruit-guide to seira-35-recruit-story');
  }
}

publishSeiraAllSupabase().catch(console.error);
