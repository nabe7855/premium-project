...
async function investigateKiseki() {
  ...
  for (const k of kisekiCasts) {
    const blogs = await prisma.$queryRaw`SELECT count(*) as count FROM blogs WHERE cast_id = ${k.id}`;
    const reviews = await prisma.$queryRaw`SELECT count(*) as count FROM reviews WHERE cast_id = ${k.id}`;
    const schedules = await prisma.$queryRaw`SELECT count(*) as count FROM schedules WHERE cast_id = ${k.id}`;
