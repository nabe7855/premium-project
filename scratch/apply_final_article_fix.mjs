...
async function main() {
  ...
  const updatedTitle = '【福岡店】新人セラピスト「りく」「青空（せいら）」が入店しました';

  const updatedDescription = `いつもストロベリーボーイズ福岡店をご利用いただき、誠にありがとうございます。このたび福岡店に、新人セラピスト「りく」と「青空（せいら）」の2名が入店いたしました。

りく（癒し系新人セラピスト）

国家資格を持ち、指圧やオイルマッサージの技術に確かな自信を持つ癒し系セラピストです。レディファーストを心がけ、お客様一人ひとりの気持ちやペースを大切にしながら、その日の体調や気分に合わせた施術をご提案いたします。「頑張っているからこそ、誰かの前では無理をしてしまう」——そんな日も、飾らずありのままで過ごせる時間をお届けします😊
▶ りくのプロフィール https://www.sutoroberrys.jp/store/fukuoka/cast/-25469e

青空（せいら）（エロさ満点。期待の大型新人）
...
お二人とも初めてのお客様も大歓迎で、博多・天神・中洲エリアのホテルへの出張にも対応しています。ぜひこの機会に、新しい出会いをお楽しみください。
...
お二人の出勤日は[出勤スケジュール](https://www.sutoroberrys.jp/store/fukuoka/schedule)からご確認いただけます。はじめてのご利用をご検討中の方は[初めての方へのご案内](https://www.sutoroberrys.jp/store/fukuoka/first-time)を、コースと料金は[料金システム](https://www.sutoroberrys.jp/store/fukuoka/price)をご覧ください。ご予約・お問い合わせを心よりお待ちしております。`;
  ...
  const updatedSections = sections.map((sec) => {
    if (sec.type === 'hero' && sec.content) {
      return {
        ...
        content: {
          ...sec.content,
          title: '【福岡店】新人セラピスト「りく」「青空（せいら）」が入店しました',
  ...
  const result = await prisma.pageRequest.update({
    where: { slug: targetSlug },
    data: {
      title: updatedTitle,
      status: 'published',
  ...
  console.log('✅ Article successfully updated in DB!');
