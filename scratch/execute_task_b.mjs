...
async function executeTaskB() {
  ...
  const oldKisekiSlug = '-348075';
  const newKisekiSlug = '-c9616d';

  // 1. 旧アカウント全データ取得（バックアップ）
  const oldCast = await prisma.$queryRaw`SELECT * FROM casts WHERE id = ${oldKisekiId}::uuid`;
  const oldMemberships = await prisma.$queryRaw`SELECT * FROM cast_store_memberships WHERE cast_id = ${oldKisekiId}::uuid`;
  const oldBlogs = await prisma.$queryRaw`SELECT * FROM blogs WHERE cast_id = ${oldKisekiId}`;
  const oldReviews = await prisma.$queryRaw`SELECT * FROM reviews WHERE cast_id = ${oldKisekiId}`;
  const oldSchedules = await prisma.$queryRaw`SELECT * FROM schedules WHERE cast_id = ${oldKisekiId}`;
  ...
  await prisma.$executeRaw`
    UPDATE casts SET is_active = false WHERE id = ${oldKisekiId}::uuid
  `;
